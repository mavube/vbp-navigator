...
export async function POST() {
  ...
  try {
    ...
    // single generic message — this app is internal to a 5-person org,
    // not customer-facing, so "401 authentication_error" or "402
    // billing_error" shown directly saves a trip to Vercel's function
    // logs to find out the key is wrong or billing isn't set up, which
    // is the single most likely first-run failure mode for this phase.
    if (err instanceof APIError) {
      return NextResponse.json(
        { error: `The AI service returned an error (${err.status} ${err.type ?? "unknown"}): ${err.message}` },
        { status: 502 }
      );
    }
