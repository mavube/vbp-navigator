...
## v2.0 status: Foundation through Phase 7 — full roadmap complete
...
**Phase 5 (Budget + Compensation Earning Service)**, **Phase 6 (Service
Requests + Collaboration)**, and **Phase 7 (Capabilities/Workload +
Outcomes metrics)** are all in — see their sections below. That's every
phase on the originally-confirmed roadmap; what's left is the "Not done
yet" follow-ups noted per phase (mostly: a real Supabase project +
migrations + seed data, a real invite flow, a couple of editing UIs) and
the cross-cutting items at the end of the build guide. Full detail:
...
`/capabilities` — the reporting layer, and the last phase on the v2.0
roadmap. No new persisted data model: everything shown here is computed
live, on every request, by grouping the six phases already built
(tasks, service requests, leads, classes, budget requests, expenses,
compensation entries) by `service_id` — see `lib/rollups.ts`. That
means the numbers can never go stale, at the cost of doing the
aggregation on every page load rather than once in the background;
fine at this data volume, worth revisiting if a tenant's data grows
large enough to matter.

Two tabs, two lenses on the same underlying counts:

- **Workload** — what's active right now. Per service (open tasks,
  open requests, active leads, active classes — services with nothing
  active are summarized as a count rather than listed one by one) and
  per person (open + in-progress tasks by assignee — Tasks-only for
  now, since it's the only module with a "who's doing this" field so
  far; see the build guide's Phase 7 "Not done yet" for extending this).
- **Outcomes** — what's actually been delivered. Each service's static
  outcome sentence (`services.outcome` — present in the schema since
  Phase 1 but not actually surfaced anywhere until this phase) paired
  with computed totals: tasks done, requests resolved, leads admitted,
  classes completed, budget approved, expenses, and compensation
  finalized/net pay.

Every rollup query is written to run identically on both the Postgres
and SQLite drivers (`SUM(CASE WHEN ... THEN 1 ELSE 0 END)` instead of
Postgres's `FILTER` clause), and every service gets the exact same
metrics computed regardless of its type — no hardcoding to a specific
service by name, the same discipline as Pipeline (Phase 3) and Classes
(Phase 4), so this keeps working unchanged for a future tenant with a
completely different service catalog.
...
app/
  ...
  capabilities/page.tsx                     — v2.0 Phase 7: the Capabilities page (Workload/Outcomes tabs)
  api/
    ...
    rollups/route.ts                                                — GET { services, byPerson } — the Phase 7 reporting layer, no new tables
components/
  ...
  capabilities/                                       — v2.0 Phase 7: CapabilitiesWorkspace (tabs), WorkloadView, OutcomesView, types
lib/
  ...
  rollups.ts                                    — v2.0 Phase 7: getServiceRollups/getPersonWorkload, cross-driver-portable grouped aggregation over the six phases above, no new tables
